...
function ErrCard({ code, message, onDismiss }) {
  return (
    <div style={{ background:T.redDim, border:"1px solid #fecaca", borderRadius:6, padding:"10px 13px", marginBottom:13 }} className="fade-in">
    ...
    <div className="fade-in">
      ...
      <Card>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
          <span style={{ fontWeight:600, fontSize:14, color:T.text }}>Recent Imports</span>
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      const fnRes = await fetch(`${SUPABASE_URL}/functions/v1/send-otp-email`, {
        ...
        body: JSON.stringify({ email: user.email, mobile: mob, otp: code }),
      });
      const fnData = await fnRes.json();
      if (!fnRes.ok || fnData.error) {
        throw new Error(fnData.error || "Failed to send email. Please try again.");
      }
      setOtpSent(true);
      setOtpErr("");
        ...
        <Card style={{ gridColumn:"1 / -1" }}>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              >
                {otpSending ? "Sending…" : mobileVerified ? "Verified" : otpSent ? "Resend OTP" : "Send Email OTP"}
              </Btn>
            </div>
            ...
            {otpSent && !mobileVerified && (
              <div style={{ marginTop:10, padding:"14px 16px", background:T.accentDim, border:`1px solid ${T.accent}33`, borderRadius:10 }}>
                <p style={{ fontSize:12, color:T.textMid, marginBottom:10 }}>Enter the 6-digit OTP sent to <strong>{user?.email}</strong></p>
                ...
                <p style={{ fontSize:11, color:T.textDim, marginTop:8 }}>OTP valid for 10 minutes · Sent free via email · Check spam if not received</p>
              </div>
            )}
          </div>

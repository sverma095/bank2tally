...
function LoginScreen({ onLogin }) {
  ...
  const handleRegister = async () => {
    ...
    if (!regEmail.includes("@"))   return setErr("Enter a valid email.");
    ...
    try {
      ...
      // Profile + approval request created automatically by DB trigger
      setPendingUser({ name: regName, email: regEmail, status: "pending" });
    } catch (e) { setErr(e.message); }
    setLoading(false);
  };
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const rows = await sb.from(
        "approval_requests",
        "status=eq.pending&select=*,profiles!approval_requests_user_id_fkey(name,email,company,created_at)&order=requested_at.asc"
      );
      setRequests(rows);
      ...
      sb.from("profiles", "id=eq." + session.user.id + "&select=*")
        .then(profiles => {
          ...
          if (profile && profile.status === "approved") {
            // email lives in session, not profiles table
            onLogin({ ...profile, email: session.user.email, sessionToken: session.access_token });
          } else if (!profile) {
            // No profile — clear session
            localStorage.removeItem("sb_session");
          }
        })
